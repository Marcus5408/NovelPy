...
class VisNovel:
    def __init__(self, configs: Dict) -> None:
        ...
        self.config = configs
        pg.init()
        self.config["default_font"] = self.config[
            "default_font"] if "default_font" in self.config else "Roboto-Regular.ttf"
    ...
    def mainloop(self) -> None:
        while self.running:
            ...
            for action in self.game_queue:
                if action["ulid"] not in [current_action["ulid"] for current_action in self.current_queue]:
                    self.current_queue.append(action)

            for action in self.current_queue:
                if action["time_started"] is not None:
                    if pg.time.get_ticks() - action["time_started"] > action["duration"] * 1000:
                        action = None
                self.screen.blit(action["surface"], (0, 0))
                action["time_started"] = pg.time.get_ticks() if action["time_started"] else action["time_started"]
                pg.display.flip()
            self.current_queue = [action for action in self.current_queue if action is not None]
    ...
    def text_screen(self,
        ...
        # center the text
        text_rect.center = (self.config["screen_size"][0] // 2, self.config["screen_size"][1] // 2)
